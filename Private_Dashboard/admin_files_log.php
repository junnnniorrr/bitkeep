...
<head>
  ...
  <title>BitKeep Management System - Admin File Logs</title>
  <!-- Font Awesome -->
...
<body>
  ...
  <div class="sidebar">
    ...
    <ul class="nav flex-column mt-3">
      ...
      <li class="nav-item">
        <a class="nav-link active" href="admin_files_log.php">
          <i class="fas fa-file-alt"></i>
          <span>Admin File Logs</span>
        </a>
      </li>
      <li class="nav-item">
  ...
  <div class="main-content">
    ...
    <div class="content-card fade-in">
      <div class="card-header">
        ...
        <?php
        $query = mysqli_query($conn,"SELECT COUNT(*) as total FROM admin_file_logs") or die (mysqli_error($conn));
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table id="adminFileLogsTable" class="table table-striped table-hover">
            <thead>
              <tr>
                <th>Log ID</th>
                <th>Admin</th>
            ...
            <tbody>
              <?php 
              $query = mysqli_query($conn,"SELECT l.*, f.name as file_name, f.file_path 
                                           FROM admin_file_logs l
                                           LEFT JOIN folder_files f ON l.file_id = f.id
                                           ORDER BY l.timestamp DESC") or die (mysqli_error($conn));
              while($log = mysqli_fetch_array($query)){
                $log_id = $log['log_id'];
                $admin_email = $log['admin_email'];
                $file_id = $log['file_id'];
                $file_name = isset($log['file_name']) ? $log['file_name'] : 'File not found';
                $folder_id = $log['folder_id'];
                $action = $log['action'];
                $timestamp = $log['timestamp'];
                $ip_address = $log['ip_address'];
                
                // Determine file icon class based on file extension
                $icon_class = 'fa-file';
                $type_class = '';
                
                if (isset($log['file_name'])) {
              ...
              <tr>
                ...
                <td>
                  <span class="badge bg-light text-dark">
                    <?php echo htmlspecialchars($folder_id); ?>
                  </span>
                </td>
                <td>
                ...
                <td>
                  <div class="d-flex align-items-center">
                    ...
                    <?php echo date('M d, Y g:i A', strtotime($timestamp)); ?>
                  </div>
                </td>
                <td>
                  <small class="text-muted"><?php echo htmlspecialchars($ip_address); ?></small>
                </td>
              </tr>
              <?php } ?>
            </tbody>
  ...
  <script>
    $(document).ready(function() {
      ...
      var table = $('#adminFileLogsTable').DataTable({
        responsive: true,
        language: {
          search: "_INPUT_",
        ...
        order: [[5, 'desc']] // Order by timestamp, newest first
      });
      ...
      $('#exportBtn').click(function(e) {
        ...
        // Get headers
        $('#adminFileLogsTable thead th').each(function() {
          headers.push($(this).text().trim());
        });
        
        // Get data rows
        $('#adminFileLogsTable tbody tr').each(function() {
          var row = [];
          $(this).find('td').each(function(index) {
            ...
            if (index === 1) { // Admin column
              cellText = $(this).text().trim();
            } else if (index === 2) { // File column
        ...
        link.setAttribute("download", "admin_file_logs_" + new Date().toISOString().slice(0,10) + ".csv");
        ...
        Toastify({
          text: "Admin file logs exported successfully",
          duration: 3000,
          close: true,
          gravity: "top",
          position: "right",
